...
router.post('/create', verifyUser, async (req, res, next) => {
    ...
    try {
        ...
        if (result.sandbox.previewUrl) {
            console.log(`Preview URL: ${result.sandbox.previewUrl}`);
        } else {
            console.log("Agent finished, but no URL was returned.");
        }
        
        const generatedFiles = await sbx.files.list('/home/user/app/src/components');
        console.log("Generated Components:", generatedFiles.map(f => f.path));
    } catch (e) {
        console.log(e);
    }
...
router.get('/:projectId/files', verifyUser, async (req, res) => {
    ...
    try {
        // Fetch files using Drizzle
        const filesList = await db.select().from(files).where(eq(files.projectId, projectId));
